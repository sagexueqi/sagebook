...
悬挂: _调用方在Try阶段由于网络抖动超时调用失败，服务方也暂时没有收到Try请求；进而会尝试执行Cancel动作，释放资源(1、2阶段)完成后，服务提供方又收到了Try请求(3阶段)_

![分布式事务_TCC_空回滚和防悬挂](./imgs/分布式事务_TCC_空回滚和防悬挂.png)

**a. 基于冻结流水号进行Cancel的方案**
...
- 在Try阶段，以业务方单号作为冻结维度，而不是以服务提供方（例如账户系统）模型（例如账号）的维度为准；因为本地TCC事务协调器，一定是嵌入在业务方系统中的，它的唯一标示只有业务方的单号
