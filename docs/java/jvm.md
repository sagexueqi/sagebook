...
- 内存模型：
  ...
  ![jvm_cms](./imgs/jvm_g1_memory.jpg)
  - 每个Region被标记了E、S、O和H，E代表的是Eden，S代表的是Survivor，H代表的是Humongous，O代表的是老年代，灰色的代表的是空闲的region
  - Region可以说是G1回收器一次回收的最小单元,每一次的回收，G1会选择可能回收最多垃圾的Region进行回收
- 工作流程: 

  ![jvm_cms](./imgs/jvm_g1.png)

**G1与CMS收集器的对比**
- 作用范围不同: CMS作用于年老代；G1作用于整个堆空间且无需与其他垃圾收集器配合
- 回收算法不同: CMS基于`标记-清除`；G1从整体上看基于`标记-整理`，在两个Region中是基于`标记-复制`算法
